...
<head>
    ...
    <script src="assets/js/cursor.js?ts=<?=time ()?>"></script>
...
>
    ...
    <div class="settings-div fadeInOut z-30 absolute top-6 left-0 right-0 flex items-center justify-center">
        <div class="flex flex-row gap-2 px-4 py-2 bg-white/20 text-white rounded-full">
    ...
    <div class="h-full w-full flex align-center justify-center z-20">
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-12 justify-center items-center px-6 lg:px-12 xl:px-0 w-full xl:w-5/6">
            <img
                ...
                class="rounded-2xl h-auto w-[30rem] custom-img-shadow"
            >

            <div class="flex flex-col lg:gap-1 xl:gap-2 w-full text-white">
                <h1 x-text="$store.player.playbackObj.item?.name ?? translations.defaultTitleSong" class="text-4xl lg:text-7xl font-bold"></h1>
                <h2 x-text="$store.player.playbackObj.item?.artists?.map(artist => artist.name).join(', ') ?? translations.defaultArtistSong" class="text-2xl lg:text-5xl font-bold"></h2>
                <h3 x-text="$store.player.playbackObj?.item?.album?.name" class="text-xl lg:text-4xl font-semibold opacity-80"></h3>
                ...
                <div class="flex flex-col gap-2 lg:gap-3 mt-4 lg:mt-8 w-full">
                    <div class="text-xl flex flex-row justify-between w-full font-semibold">
                        <span x-text="msToTime($store.player.playbackObj?.progress_ms)"></span>
                        <span x-text="msToTime($store.player.playbackObj?.item?.duration_ms)"></span>
                    </div>
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
        ...
        const player = new Spotify.Player({
            name: 'NowPlaying #' + localStorage.getItem('deviceId'),
